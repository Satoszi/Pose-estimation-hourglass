...
class ModelManager():
    ...
    def unet_conv(self, inputs, filters, dropout=0.3,kernel_size=3, padding="same", strides=2, activation=True):
        x = Conv2D(
            filters=filters,
    ...
    def unet_up_conv(self, inputs, filters, dropout=0.3,kernel_size=3, padding="same", strides=2, activation=True):
        x = Conv2DTranspose(
            filters=filters,
            kernel_size=kernel_size,
            kernel_initializer=self.w_init,
    ...
    def build_model_hourglass_2_blocks(self, img_size=64, num_channels=3):

        inputs = keras.Input(shape=(img_size,img_size,num_channels,))
        x1 = self.unet_conv(inputs, filters = 32, strides = 1)
        
        x2 = self.unet_conv(x1, filters = 32, strides = 2)
        x3 = self.unet_conv(x2, filters = 32, strides = 1)
        
        x4 = self.unet_conv(x3, filters = 64, strides = 2)
        x5 = self.unet_conv(x4, filters = 64, strides = 1)
        
        x6 = self.unet_conv(x5, filters = 128, strides = 2)
        x7 = self.unet_conv(x6, filters = 128, strides = 1)
        
        
        x8 = self.unet_conv(x7, filters = 256, strides = 2)
        x9 = self.unet_conv(x8, filters = 256, strides = 1)
        
        x10 = self.unet_up_conv(x9, filters = 256, strides = 2)
        
        
        x = layers.concatenate([x7, x10])
        x11 = self.unet_conv(x, filters = 256, strides = 1)
        x12 = self.unet_up_conv(x11, filters = 256, strides = 2)
        
        x = layers.concatenate([x5, x12])
        x13 = self.unet_conv(x, filters = 128, strides = 1)
        x14 = self.unet_up_conv(x13, filters = 128, strides = 2)
        
        x = layers.concatenate([x3, x14])
        x15 = self.unet_conv(x, filters = 64, strides = 1)
        x16 = self.unet_up_conv(x15, filters = 64, strides = 2)
        
        x = layers.concatenate([x1, x16, inputs])
        x17 = self.unet_conv(x, filters = 64, strides = 1)
        ...
        x1 = self.unet_conv(x, filters = 32, strides = 1)
